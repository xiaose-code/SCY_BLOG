...
cd /data/tq_usb/usbguard/do_build && ../init_build.sh
...
如果没有，则需要 ssh 远程连接其他终端上通过内网拉取，拷贝到这个目录，然后修改路径 `sudo vim libhv.pc` 其他三方库出现这个问题也一样。如下路所示，把下图的 prefix 的路径改为自己项目所在的路径
...
运行脚本前先看看路径，因为不同终端的编译路径可能会不一样。

这个是 `/data/tq_usb/usbdesktop/` 目录下的 `run.sh` 文件

![1705386595330](https://cdn.jsdelivr.net/gh/xiaose-code/Pictures@main/img/1705386595330.webp)

在看看`/data/tq_usb/usbdesktop/` 目录下的 `build.sh` 文件

![1705386997129](https://cdn.jsdelivr.net/gh/xiaose-code/Pictures@main/img/1705386997129.webp)
...
# 有的话就清空
cd do_build && rm -fr *
cd /data/tq_usb/usbdesktop/do_build && ../build.sh

# 然后执行
cd /data/tq_usb/usbdesktop && make

cd /data/tq_usb/usbdesktop/do_build && ../run.sh

./run.sh
```

笔者这里运行 `build.sh` 脚本是因为缺少了 qt5 的部分组件，所以报错了，有类似的朋友可以参考
...
然后接着继续编译源码，还是在 `do_build` 目录执行 `make`
...
同样先进入脚本查看路径，因为这个是QT编写的程序，所以这里的脚本应该是以 `.pro` 结尾的文件

![1705390104544](https://cdn.jsdelivr.net/gh/xiaose-code/Pictures@main/img/1705390104544.webp)

```bash
sudo vim prower_reg.pro
```

![1705390193134](https://cdn.jsdelivr.net/gh/xiaose-code/Pictures@main/img/1705390193134.webp)

当然，图中的路径是已经注释了的，不改也可以，只是为了一致性。
...
# 或者是在 /usr/share/polkit-1/actions/ 目录下
cd /usr/share/polkit-1/actions/
sudo cp /data/tq_usb/usbguard/src/DBus/org.usbguard1.policy .
cd /usr/share/dbus-1/system.d/
sudo cp /data/tq_usb/usbguard/src/DBus/org.usbguard1.conf .
```

## 4.5 运行项目

> 运行之前要初始化，不然开启项目后会把鼠标键盘全部禁用掉。
...
![1705395494488](https://cdn.jsdelivr.net/gh/xiaose-code/Pictures@main/img/1705395494488.webp)
...
```bash
# 首先进入 do_build下的隐藏目录 .libs
cd /data/tq_usb/usbguard/do_build/.libs
sudo ./usbguard-daemon -d

# 新开一个标签,返回 do_build 目录，执行
sudo ./usbguard-dbus -s

# 再新开一个标签，切换目录 /data/tq_usb/usbdesktop/do_build，执行 ../run.sh
...
运行 `./run.sh` 脚本时出现以下错误，对 run.sh 脚本继续修改

```bash
kylin@kylin:/data/tq_usb/usbdesktop/do_build$ ./run.sh
terminate called after throwing an instance of 'std::logic_error'
  what():  basic_string::_M_construct null not valid
./run.sh：行 9: 308527 已放弃               （核心已转储）./usbdesk -m network -c /data/tq_usb/usbdesktop/etc
```

![1705399112769](https://cdn.jsdelivr.net/gh/xiaose-code/Pictures@main/img/1705399112769.webp)

可以通过查看上级目录的 main.cpp 文件

```bash
vim ../main.cpp
```

![1705399206948](https://cdn.jsdelivr.net/gh/xiaose-code/Pictures@main/img/1705399206948.webp)

# 5. 打包成安装包

将 `/data/tq_usb/release/x86_64/kylin-v10/lib` 整个目录 copy 到 `/data/tq_usb/make_package/res/lib`目录下

```bash
cp -fr /data/tq_usb/release/x86_64/kylin-v10/lib/ .
```
...
![1705391414379](https://cdn.jsdelivr.net/gh/xiaose-code/Pictures@main/img/1705391414379.webp)

------

将 `/data/tq_usb/release/x86_64/kylin-v10/bin` 目录下的 PowerReg、usbdesk、usbguard 三个文件
copy 到 `/data/tq_usb/make_package/res/bin` 目录下

```bash
cp /data/tq_usb/release/x86_64/kylin-v10/bin/PowerReg .
cp /data/tq_usb/release/x86_64/kylin-v10/bin/usbdesk .
cp /data/tq_usb/release/x86_64/kylin-v10/bin/usbguard .
```

![1705393378879](https://cdn.jsdelivr.net/gh/xiaose-code/Pictures@main/img/1705393378879.webp)
...
> 不知道是不是 bug，这里要敲击 `ctrl+C` 程序才会继续运行，不然就会一直卡在这里。
